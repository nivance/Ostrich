...
import org.ostrich.api.framework.protocol.LoginPacket;
...
public class EntityLoginFilter extends LoginFilter {
	...
	public NextAction handleRead(FilterChainContext ctx) throws IOException {
		...
		if (packet.getPacketType() == PacketType.kickoff) {
			//kicking off
			log.warn("kick off by other:"+packet.getEntity()+",authtoken="+authO.getToken()+",priority="+authO.getPriority());
			connMan.stop();
			...
			if (AuthEntity.LoginSuccessed.equals(auth)) {
				log.debug("Client Login Successed!@" + packet.getFrom()+",token="+auth.getToken());
