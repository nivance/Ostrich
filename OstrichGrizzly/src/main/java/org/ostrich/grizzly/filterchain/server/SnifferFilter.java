...
public class SnifferFilter extends BaseFilter {
	...
	public SnifferFilter(GrizzlyServer router) {
		this.router = router;
	}
	...
	class SORunner implements Runnable {
		...
		public void run() {
			try {
				JsonPacket sendpack = JsonPacket.newPublish(
						session.getJid(), subentity.getSubkey(), packet);
	...
	public NextAction handleClose(FilterChainContext ctx) throws IOException {
		if (attrSubscription.isSet(ctx.getConnection())) {
			Sniffer sniffer = subs.get(attrSubscription.get(ctx.getConnection()));
